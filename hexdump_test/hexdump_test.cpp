...
TEST(formatBytes, when_single_byte_then_displays_single_byte_spaced_correctly)
{
	...
	EXPECT_EQ("00                                                   .", formatBytes(&bytes[0], bytes.size()));
}
...
TEST(formatBytes, displays_values_in_hex)
{
	...
	EXPECT_EQ("0A FF                                                ..", formatBytes(&bytes[0], bytes.size()));
}
...
TEST(formatBytes, when_16_bytes_then_spaced_correctly)
{
	...
	EXPECT_EQ("00 00 00 00 00 00 00 00   00 00 00 00 00 00 00 00    ................", formatBytes(&bytes[0], bytes.size()));
}
...
TEST(formatBytes, when_character_data_then_displays_printable)
{
	...
	EXPECT_EQ("54 65 73 74                                          Test", formatBytes(&bytes[0], bytes.size()));
}
...
TEST(formatLine, displays_offset_in_hex)
{
	...
	EXPECT_EQ("000A: 54 65 73 74                                          Test", formatLine(10, &bytes[0], bytes.size()));
}
